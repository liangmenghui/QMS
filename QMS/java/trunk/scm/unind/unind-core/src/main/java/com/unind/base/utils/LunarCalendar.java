...
public final class LunarCalendar extends GregorianCalendar {
	 ...
	 * @param d
	 */
	public LunarCalendar(boolean isChinese, int y, int m, int d) {
		if (isChinese) {
			set(CHINESE_YEAR, y);
			set(CHINESE_MONTH, m);
			set(CHINESE_DATE, d);
	...
	public String getSimpleGregorianDateString() {
		return new StringBuffer().append(get(YEAR)).append("-")
				.append((get(MONTH) + 1)<10?("0"+(get(MONTH) + 1)):get(MONTH) + 1).append("-").append(get(DATE)<10?("0"+get(DATE)):get(DATE))
				.toString();
	}
	...
	public String getSimpleChineseDateString() {
		return new StringBuffer()
				...
				.append("-")
				.append(get(CHINESE_MONTH) > 0 ? (get(CHINESE_MONTH)<10?("0" + get(CHINESE_MONTH)):get(CHINESE_MONTH)) : "*" + (-get(CHINESE_MONTH))).append("-")
				.append(get(CHINESE_DATE)<10?("0"+get(CHINESE_DATE)):get(CHINESE_DATE)).toString();
	}
