...
public class ApprovedFlowController extends WebController {
	...
	@Autowired
	private SysRolePubService sysRolePubService;
	...
	@ApiImplicitParams({
			@ApiImplicitParam(name = "bsType", value = "1:产品/SQE,2:供应商/SDE", dataType = "int", paramType="query",defaultValue=""),
	})
	@RequestMapping(value = "/getUserByBsType", method = RequestMethod.GET)
	public ApiResponseResult getUserByBsType(int bsType) {
		try {
			if(bsType == 1){
				return sysRolePubService.getuserbyrole("QMS_SQE");
			}else return sysRolePubService.getuserbyrole("QMS_SDE");
		} catch (BusinessException e) {
			return ApiResponseResult.failure(e.getMessage());
		}
	}
