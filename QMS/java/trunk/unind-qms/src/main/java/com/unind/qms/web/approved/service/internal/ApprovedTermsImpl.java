...
//        }
        ...
        o.setBsNameEn(approvedTerms.getBsNameEn());
...
//        o.setPkModifiedBy(Long.parseLong("1"));
    ...
    public ApiResponseResult getlist(String keyWord, Long itemsId, Long itemsRecordId, Integer bsType, PageRequest pageRequest) {
        if(null != itemsId&&null != itemsRecordId){
            ...
            sqlbf1.append(" SELECT a.*,(SELECT bs_score FROM "+ ApprovedTermsScore.TABLE_NAME+" WHERE bs_items_recore_id=:itemsRecordId and bs_terms_id=a.id) bs_score");
            sqlbf1.append(" ,(SELECT bs_desc FROM "+ApprovedTermsScore.TABLE_NAME+" WHERE bs_items_recore_id=:itemsRecordId and bs_terms_id=a.id) bs_desc");
            sqlbf1.append(" ,(SELECT id FROM "+ApprovedTermsScore.TABLE_NAME+" WHERE bs_items_recore_id=:itemsRecordId and bs_terms_id=a.id) score_id");
            sqlbf1.append(" FROM "+ApprovedTerms.TABLE_NAME+" a LEFT JOIN "+ ApprovedItemsMap.TABLE_NAME+" b ON a.id = b.bs_terms_id");
