...
//	}
	...
	@ApiOperation(value="获取条款评分", notes="获取条款评分")
	@ApiImplicitParams({
			@ApiImplicitParam(name = "bsItemsRecordId", value = "项目记录ID", dataType = "Long", paramType="query",defaultValue=""),
	})
	@RequestMapping(value = "/getlist", method = RequestMethod.GET)
	public ApiResponseResult getlist(Long bsItemsRecordId) {
		try {
			Sort sort = new Sort(Sort.Direction.ASC,"id");
			return approvedTermsScoreService.getlist(bsItemsRecordId, super.getPageRequest(sort));
		} catch (BusinessException e) {
			return ApiResponseResult.failure(e.getMessage());
		}
	}
