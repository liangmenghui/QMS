...
import com.unind.qms.web.supplier.dao.SpecialCheckSupplierInfoDao;
...
public class SupplierInfoImpl extends BaseOprService implements SupplierInfoService {
    ...
    @Autowired
    private SpecialCheckSupplierInfoDao specialCheckSupplierInfoDao;
...
//        supplierInfo.setPkCreatedBy(Long.parseLong("1"));
        ...
        supplierRisk.setBsSuppId(supplierInfo.getId());
        supplierRiskService.add(supplierRisk);
...
//        }else{}
    ...
    public ApiResponseResult getSrmApprovedFile(String suppcode) throws BusinessException {
        ...
        StringBuffer sqlbf1 = new StringBuffer();
        sqlbf1.append(" SELECT distinct a.id,to_char(a.bs_created_time,'yyyy-MM-dd') created_time FROM "+ApprovedFlowRecord.TABLE_NAME+" a");
        sqlbf1.append(" LEFT JOIN "+ApprovedFlow.TABLE_NAME+" b on a.bs_flow_id = b.id");
        sqlbf1.append(" LEFT JOIN "+ ApprovedItemsRecord.TABLE_NAME+" c on a.id = c.bs_flow_record_id");
        sqlbf1.append(" LEFT JOIN "+SupplierInfo.TABLE_NAME+" d on a.bs_supp_id = d.id");
        sqlbf1.append(" WHERE c.bs_step = (select count(1) from t_approved_recorder_map e where e.bs_items_record_id = c.id)");
        sqlbf1.append(" and b.bs_type = '22' and d.bs_supp_code = :suppcode");
	 ...
	 * 
	 */
	public ApiResponseResult doPecialCheckSupplier(SpecialCheckSupplierInfo specialCheckSupplierInfo) throws BusinessException {
		// TODO Auto-generated method stub
		
		//根据供应商编号获取供应商信息
		List<SupplierInfo> supplierInfoList = supplierInfoDao.findByBsSuppCode(specialCheckSupplierInfo.getBsSuppCode());
		if(supplierInfoList.size()==0){
            return ApiResponseResult.failure("无供应商信息！");
        }
		SupplierInfo supplierInfo = supplierInfoList.get(0);
		
		//1:记录到特批供应商记录表
		specialCheckSupplierInfo.setBsCreatedTime(new Date());//创建时间手动添加
		specialCheckSupplierInfoDao.save(specialCheckSupplierInfo);
		//2:修改供应商状态
        supplierInfoDao.updatebsStatusById(supplierInfo.getId(), "6");
        //3:修改SRM供应商的等级
        List<NameValuePair> formparams = new ArrayList<NameValuePair>();
        formparams.add(new BasicNameValuePair("login_name",supplierInfo.getBsLoginName()));
        formparams.add(new BasicNameValuePair("grade",supplierInfo.getBsSuppGrade()));
        formparams.add(new BasicNameValuePair("status","6"));

        String srmAddress = appConfig.getString("srm.address");

        ApiResponseResult result = httpClientService.postForm(srmAddress+"/UnindSRM/suppliermanage/suppbasemaneger!changeSuppByQms.action",formparams);
		
		 //4.给角色为供应商发展经理的用户发送待办事项
        //4.1找到该角色下的角色ID
        List<SysRole> sysRoleList = sysRoleDao.findByBsCode("QMS_DEVELOPMENT_MANAGER");
        if(sysRoleList.size()==0){
            return ApiResponseResult.failure("无供应商发展经理角色！");
        ...
        for(int i = 0; i < sysUserRolesAggList.size(); i++){
            ...
            todoInfo.setBsExtend(supplierInfo.getId());   //扩展内容
            todoInfo.setBsRouter("/qms/supplier/approved");  //页面路由
            todoInfo.setBsTitle("[" + supplierInfo.getBsSuppChieseName() +"]" + "待进行初期评审(特批)");   //标题
            todoInfo.setBsStatus(BasicEnumConstants.TODO_NOFINISH);  //状态
            todoInfo.setBsSystemType(BasicStateEnum.TODO_QMSTYPE.intValue());  //系统状态
            todoInfo.setBsType(BasicEnumConstants.TODO_SUPP_SETUP);   //类型
            todoInfo.setBsStartTime(new Date());      //有效开始时间
