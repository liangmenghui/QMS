...
public class ApprovedItemsController extends WebController {
	...
	public ApiResponseResult add(ApprovedItems approvedItems, String termsStr) {
		try {
			return approvedItemsService.add(approvedItems,termsStr);
	...
	public ApiResponseResult edit(ApprovedItems approvedItems, String termsStr) {
		try {
			return approvedItemsService.edit(approvedItems,termsStr);
		} catch (BusinessException e) {
			e.printStackTrace();
			return ApiResponseResult.failure(e.getMessage());
		}
