            ...
            <i-col>
                <Form class="query_area" ref="formQuery" :model="formQuery" inline>
                    <Form-item prop="bsCode">
                        <Input v-model="formQuery.bsCode" placeholder="集团编码" />
                    </Form-item>
                    <Form-item prop="bsName">
                        <Input v-model="formQuery.bsName" placeholder="集团名称" />
                    </Form-item>
        ...
        <Modal v-model="dialog.modal_dialog" title="" @on-ok="ok" @on-cancel="cancel" >
            <p>
                <Form :model="dialog.formItem" :ref="dialog.ruleForm" :rules="dialog.ruleForm" :label-width="80">
                    ...
                    <Form-item label="集团编码" prop="bsCode">
                        <Input v-model="dialog.formItem.bsCode" placeholder="请输入"></Input>
                    </Form-item>
                    <Form-item label="集团名称" prop="bsName">
...
export default {
    ...
    methods: {
        ...
        getData() {
            ...
            this.api.admin.group.getlist(this.formQuery).then((res) => {
                if(res.result) {
        ...
        edit(params) {
            this.api.admin.group.edit(params.row).then((res)=>{
                if(res.result) {
                    //refresh
                    this.reloadData();
                }else {
        ...
        delete(params) {
            this.$Modal.confirm({
                ...
                loading: true,
                onOk: () => {
                    this.api.admin.group.delete({id:params.row.id}).then((res)=>{
                        if(res.result) {
                            //refresh
                            this.reloadData();
        ...
        ok () {
            if(typeof(this.dialog.formItem.id)!=undefined && typeof(this.dialog.formItem.id)=="number") {
                ...
                //     if (valid) {
                        this.api.admin.group.edit(this.dialog.formItem).then((res) => {
                            if(res.result) {
                                //refresh
                                this.reloadData();
                            }else {
                ...
                // this.$refs["dialog.ruleForm"].validate((valid) => {
                //     if (valid) {
                        this.api.admin.group.add(this.dialog.formItem).then((res) => {
                            if(res.result) {
                                //refresh
                                this.reloadData();
                            }else {
