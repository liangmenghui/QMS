...
<script>
export default {
    props:['approvedItemRecord','supplierInfo','productInfo','flowData','records'],
    data() {
        return {
                ...
                this.api.Audit.approved(params).then((res) => {
                    ...
                    this.api.approvedFlowRecord.edit({bsResult:flowResult, id:this.$route.query.bsFlowRecordId}).then((res) => {
                        ...
                        if(restart){
							...
							for(var i = 0;i<this.records[0].userids.length-1;i++){
								if(i == 0){
									createRecordData.recorderStr = this.records[0].userids[i];
								}else createRecordData.recorderStr += ',' + this.records[0].userids[i];
							}
							...
							this.api.todo.addReview({bsSuppId:this.supplierInfo.id}).then((res) => {});
                        }
                    });
                });
            }
