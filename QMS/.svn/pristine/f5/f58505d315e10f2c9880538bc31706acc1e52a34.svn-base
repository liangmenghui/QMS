...
//			@ApiImplicitParam(name = "prodResult", value = "成品检验核查结果", dataType = "Integer", paramType="query",defaultValue=""),
//	})
//	@RequestMapping(value = "/approved", method = RequestMethod.POST)
//	public ApiResponseResult approved(Long bsItemsId, Long bsItemsRecordId, int bsResult, String resultDesc, String itemsDesc, Integer prodResult) {
//		try {
//			return approvedItemsRecordService.approved(bsItemsId, bsItemsRecordId, bsResult, resultDesc, itemsDesc, prodResult);
...
//	}
	...
	@ApiImplicitParams({
			...
			@ApiImplicitParam(name = "bsResult", value = "审批类型（1:通过，2：驳回，3：不通过）", dataType = "int", paramType="query",defaultValue=""),
			@ApiImplicitParam(name = "bsConclusion", value = "审核结论", dataType = "Integer", paramType="query",defaultValue=""),
	})
	@RequestMapping(value = "/approved", method = RequestMethod.POST)
	public ApiResponseResult approved(Long bsFlowRecordId, int bsResult, Integer bsConclusion) {
		try {
			return approvedItemsRecordService.approved(bsFlowRecordId, bsResult, bsConclusion);
		} catch (BusinessException e) {
			return ApiResponseResult.failure(e.getMessage());
		}
	}
