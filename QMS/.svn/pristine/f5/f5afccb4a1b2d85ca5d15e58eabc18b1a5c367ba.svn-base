<template>
	...
	<div class="productinspectapporved">
		<Row :style="{marginBottom: '20px'}">
			...
			<el-button class="btn" type="primary" plain size="small" icon="el-icon-warning" @click='exportExcel()' v-if="perms.confirm" style="position: absolute;right: 30px;top: 0px;">
				{{$t('approved.export')}}
			</el-button>
		</Row>
		<Row :style="{marginBottom: '20px'}">
    		...
    		<Collapse accordion v-model="expandedCollapses[5]">
				<Panel name="1">
					...
					<div id="otherbody" slot="content">
						 <otherattachments :shipmentInspectId="shipmentInspectData.id" :fileList="shipmentInspectData.fileSet" :canUpload="perms.edit" v-if="shipmentInspectData.id != undefined">                 
						 </otherattachments>
					</div>
				</Panel>
			</Collapse>
		...
		<Row :style="{marginBottom: '20px'}" v-if = "perms.confirm">
 			<Card>
                ...
                <el-button slot="extra" type="primary" icon="el-icon-plus" style="padding: 7px 20px;" @click="addConfirmination" v-if="perms.edit">
  		        ...
  		        <div style="padding-left: 5%;" v-for="(item, index) in confirmInfo">
					...
					<div slot="content">
						<shipmentRecordFile :shipmentRecordId="item.id" :fileList="item.fileSet" :canUpload="perms.edit" v-if="index==2">                 
						</shipmentRecordFile>
					</div>
...
import shipmentRecordFile from '../../components/shipmentRecordFile.vue';
    ...
    methods: {
    	 getInspectData(){
    	 	...
    	 	this.api.shipmentInspect.getlist(params).then((res) => {
    	 		if(res.data.rows.length) {
					...
					if(this.shipmentInspectData.bsSamplesNum == 0){
		...
		getSampleNumData(){
			...
			this.api.sampleLevelCodeNum.getSampleNumber({bsRiskLevel:productLevel,bsTotal:this.shipmentInspectData.bsProductNum}).then((res) => {
				...
				this.shipmentInspectData.bsInspectStandard = res.data.bsStandardName;
				this.shipmentInspectData.bsInspectLevel = res.data.bsLevelName;
				this.shipmentInspectData.bsAqlLevel = res.data.bsLevelName; 
				this.shipmentInspectData.bsAcceptNum = res.data.bsScore;
				this.shipmentInspectData.bsRejectNum = res.data.bsScore + 1;
			});
		},
		addConfirmination(){
		...
		exportExcel() { 
            this.api.fileQms.getShipmentExcel({bsShipmentId:this.shipmentInspectData.id}).then((res) => {                
            })
        }
    }
}
