...
export default {
  ...
  methods: {
    ...
    onRowClick(row, index) {
      ...
      if (
        this.textField &&
        Object.keys(row).filter(item => row[item] === this.textField)
      ) {
        this.query = this.values[this.textField];
      } else if (
        this.modelField &&
        Object.keys(row).filter(item => row[item] === this.modelField)
      ) {
        this.query = row[this.modelField];
      } else {
        this.query = row[Object.keys(row)[0]];
      }
...
.ivu-grid-search-wrap .ivu-modal-body {
  padding: 0 2px;
}
.ivu-grid-search-wrap .ivu-modal-footer{
  border-top: none;
  padding: 5px;
}
</style>