...
//        }
        ...
        for(int i=0;i<sampleRegularRecordList.size();i++){
            ...
            if(null == sampleRegularRecord.getId()){//新增
                ...
                SampleRegular sampleRegular = sampleRegularDao.findOne(sampleRegularRecord.getBsSampleRegularId());

                //-1:go,-2:no go
                if(sampleRegularRecord.getBsMeasureResult() == null){
                    sampleRegularRecord.setBsIsResult(0);
                }else if(Float.compare(sampleRegularRecord.getBsMeasureResult(), -1) == 0){
                    sampleRegularRecord.setBsIsResult(1);
                }else if(Float.compare(sampleRegularRecord.getBsMeasureResult(), -2) == 0){
                    sampleRegularRecord.setBsIsResult(2);
                }else{
                    int upResult =Float.compare(sampleRegularRecord.getBsMeasureResult(), sampleRegular.getBsUpLimit());
                    int lowResult =Float.compare(sampleRegularRecord.getBsMeasureResult(), sampleRegular.getBsLowLimit());
                    if(upResult<=0 && lowResult>=0){
                        sampleRegularRecord.setBsIsResult(1);
                    }else{
                        sampleRegularRecord.setBsIsResult(2);
                    }
                ...
                o.setBsIsResult(sampleRegularRecord.getBsIsResult());
