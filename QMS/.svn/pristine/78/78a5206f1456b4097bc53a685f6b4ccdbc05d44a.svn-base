	...
	<div class="layout-content">	
		<div style="padding-bottom:5px;border-bottom: 1px solid #ebeef5;">
      	...
      	<el-table :data="CustormerTable" class="custmoerTable">
           ...
           <span class="circle"  v-bind:class="{bggreen:scope.row.bsResult==1 ,bgorange:scope.row.bsResult==2}"></span>

			<span v-bind:class="{green:scope.row.bsResult==1 ,orange:scope.row.bsResult==2}">{{$t('approved.customer_result['+scope.row.bsResult+']')}}</span>
   		...
   		<Modal v-model="modal_dialog" :title="$t('approved.AddCustomerReview')" @on-ok="saveHandler">
            <div class="custmode">
                <el-form label-width="120px" v-model="formItem" :ref="formItem">               
                    <el-form-item :label="$t('ApprovedFlow.AuditResults')">
					<el-select  v-model="formItem.bsResult" placeholder="请选择">
		...
		modal_dialog:false,
		 ...
		 formItem:{
		   ...
		   perms: {
            ...
            Options:[
            	{label:this.$t("approved.customer_result[0]"),value:0},
				      {label:this.$t("approved.customer_result[1]"),value:1},
				      {label:this.$t("approved.customer_result[2]"),value:2},			
            ],
            value:0,
            custmerInfo:{}
    	...
    	getlist(){ 
            if(this.$route.path.includes('supplier'))  {
                ...
                for(var i = 0; i < res.data.rows.length; i++){
                	if(res.data.rows[i].fileSet != undefined){
						      this.fileList = res.data.rows[i].fileSet.map(function (file) {
