...
export default {
    ...
    data() {
        return {
            ...
            datagrid: {
                ...
                columns: [
                    {
                        title:  this.$t('systemInfo.globalAppConfiguration.applyName'),
                        key: 'bsApplicationName',
                        align: 'center'
                    },
                    {
                        title: this.$t('systemInfo.globalAppConfiguration.type'),
                        key: 'bsCategory',
                        align: 'center'
                    },
                    {
                        title: this.$t('systemInfo.globalAppConfiguration.code'),
                        key: 'bsCode',
                        align: 'center'
                    },
                    {
                        title:  this.$t('systemInfo.globalAppConfiguration.name'),
                        key: 'bsName',
                        align: 'center'
                    },
                    {
                        title: this.$t('systemInfo.globalAppConfiguration.value'),
                        key: 'bsValue',
                        align: 'center'
                    },
                    {
                        title: this.$t('systemInfo.globalAppConfiguration.state'),
                        key: 'bsEnableState',
                        align: 'center'
                    },
                    {
                        title:  this.$t('systemInfo.globalAppConfiguration.applicationEnv'),
                        key: 'bsEnv',
                        align: 'center'
                    },
                    {
                        title: this.$t('systemInfo.globalAppConfiguration.description'),
                        key: 'bsComment'
                    },
                    {
                        title:  this.$t('systemInfo.globalAppConfiguration.action'),
                        key: 'action',
                        render: (h, params) => {
                            let ary = [];
    ...
    methods: {
        ...
        getData() {
            ...
            this.api.admin.globalconfig.getlist(this.formQuery).then((res) => {
                if(res.result) {
        ...
        edit(params) {
            this.api.admin.globalconfig.edit(params.row).then((res)=>{
                if(res.result) {
                    //refresh
                    this.reloadData();
                }else {
        ...
        delete(params) {
            this.$Modal.confirm({
                title: this.$t('model.deleteTitel'),
                content: this.$t('model.deleteContent'),
                loading: true,
                onOk: () => {
                    this.api.admin.globalconfig.delete({id:params.row.id}).then((res)=>{
                        if(res.result) {
                            //refresh
                            this.reloadData();
                            this.$Message.info($t('message.sucess'));
                            this.$Modal.remove();
                        }else {
                            this.$Message.error(res.msg);
                        }
        ...
        ok () {
            if(typeof(this.dialog.formItem.id)!=undefined && typeof(this.dialog.formItem.id)=="number") {
                ...
                //     if (valid) {
                        this.api.admin.globalconfig.edit(this.dialog.formItem).then((res) => {
                            if(res.result) {
                                //refresh
                                this.reloadData();
                            }else {
                ...
                // this.$refs["dialog.ruleForm"].validate((valid) => {
                //     if (valid) {
                        this.api.admin.globalconfig.add(this.dialog.formItem).then((res) => {
                            if(res.result) {
                                //refresh
                                this.reloadData();
                            }else {
