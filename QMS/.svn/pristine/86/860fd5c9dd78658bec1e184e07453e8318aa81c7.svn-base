            ...
            <i-col>
                <Form class="query_area" ref="formQuery" :model="formQuery" inline>
                    <Form-item prop="bsName">
                        <Input v-model="formQuery.bsName" placeholder="字段名称" />
                    </Form-item>
                    <Form-item prop="bsLabel">
                        <Input v-model="formQuery.bsLabel" placeholder="标签名称" />
                    </Form-item>    
                    <Form-item>
            ...
            <p>
                <Form :model="dialog.formItem" :ref="dialog.ruleForm" :rules="dialog.ruleForm" :label-width="80">
                    ...
                    <Form-item label="顺序号" prop="bsSortNo">
                        <InputNumber style="width:100%" v-model="dialog.formItem.bsSortNo" placeholder="请输入" value="1" ></InputNumber>
                    </Form-item>
                </Form>
            </p>
        </Modal>
...
export default {
    ...
    methods: {
        getcataloglist() {
            this.api.admin.custom.getlist({}).then((res) => {
                if(res.result) {
        ...
        init() {
            //console.log(this.$route.query.bsCode);
        ...
        getData() {
            ...
            this.api.admin.customfield.getlist(this.formQuery).then((res) => {
        ...
        edit(params) {
            //console.log(params);
            this.api.admin.customfield.edit(params.row).then((res)=>{
                //console.log(res)
                if(res.result) {
                    //refresh
                    this.reloadData();
        ...
        delete(params) {
            ...
            this.$Modal.confirm({
                ...
                loading: true,
                onOk: () => {
                    this.api.admin.customfield.delete({id:params.row.id}).then((res)=>{
                        //console.log(res)
                        if(res.result) {
                            //refresh
                            this.reloadData();
        ...
        ok () {
            ...
            if(typeof(this.dialog.formItem.id)!=undefined && typeof(this.dialog.formItem.id)=="string") {
                ...
                //     if (valid) {
                        this.api.admin.customfield.edit(this.dialog.formItem).then((res) => {
                            if(res.result) {
                                //refresh
                                this.reloadData();
                            }else {
                ...
                // this.$refs["dialog.ruleForm"].validate((valid) => {
                //     if (valid) {
                        this.api.admin.customfield.add(this.dialog.formItem).then((res) => {
                            if(res.result) {
                                //refresh
                                this.reloadData();
                            }else {
