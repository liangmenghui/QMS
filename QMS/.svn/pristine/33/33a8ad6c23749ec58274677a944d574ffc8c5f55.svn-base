            ...
            <i-col>
                <Form class="query_area" ref="formQuery" :model="formQuery" inline>
                    <Form-item prop="bsCode">
                        <Input type="text" v-model="formQuery.bsCode" placeholder="帐号" />
                    </Form-item>
                    <Form-item prop="bsName">
                        <Input type="text" v-model="formQuery.bsName" placeholder="姓名" />
                    </Form-item>
...
export default {
    ...
    methods: {
        ...
        getData() {
            ...
            this.api.user.getlist(this.formQuery).then((res) => {
        ...
        edit(params) {
            console.log(params);
            this.api.user.edit(params.row).then((res)=>{
                console.log(res)
                if(res.result) {
                    //refresh
                    this.reloadData();
        ...
        delete(params) {
            ...
            this.$Modal.confirm({
                ...
                loading: true,
                onOk: () => {
                    this.api.user.delete({id:params.row.id}).then((res)=>{
                        console.log(res)
                        if(res.result) {
                            //refresh
                            this.reloadData();
        ...
        showEditDialog(params) {
            this.dialog.modal_dialog = true;
            let r = params.row;
            this.dialog.formItem = {id:r.id,bsCode:r.bsCode, bsName:r.bsName, bsMobile:r.bsMobile, bsEmail:r.bsEmail, bsComment:r.bsComment};
        },
        ok() {
            ...
            if(typeof(this.dialog.formItem.id)!=undefined && typeof(this.dialog.formItem.id)=="number") {
                ...
                //     if (valid) {
                        this.api.user.edit(this.dialog.formItem).then((res) => {
                            if(res.result) {
                                //refresh
                                this.reloadData();
                            }else {
                ...
                // this.$refs["dialog.ruleForm"].validate((valid) => {
                //     if (valid) {
                        this.api.user.add(this.dialog.formItem).then((res) => {
                            if(res.result) {
                                //refresh
                                this.reloadData();
                            }else {
        ...
        getlist(pkUser) {
            this.api.userrole.getlist({pkUser:pkUser}).then((res) => {
                if(res.result) {
                    console.log('Tree..')
        ...
        save() {
            ...
            this.api.userrole.save({pkUser: this.roles.formItem.pkUser, pkRoles:pkRoles.toString()}).then((res) => {
                if(res.result) {
