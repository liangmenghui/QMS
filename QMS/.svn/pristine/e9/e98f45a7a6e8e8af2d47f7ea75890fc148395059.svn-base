            ...
            <i-col>
                <Form class="query_area" ref="formQuery" :model="formQuery" inline>
                    ...
                    <Form-item v-if="menuData.perms.ADD">
                        <Button type="primary" @click="runErpPoPlan()">
                            <span >需求拉取</span>
                        </Button>
                    </Form-item>
                    <Form-item>
...
export default {
    ...
    methods: {
        ...
        runCabinetsPlan(){
            this.loading2 = true;
            this.dialog.modal_dialog = false;
            this.api.pmp.runCabinetsPlan(this.selectData,this.loading2,function(res){
                return false;
            }).then((res) => {
                if(res.result) {
        ...
        runErpPoPlan(){
            this.api.pmp.runErpPoPlan(this.formQuery,this.loading2,function(res){
                    return false;
            }).then((res) =>{
                if(res.result){
                    this.$Message.success({
