...
public class PromoteController extends WebController {
    ...
    public ApiResponseResult add(Promote promote){
        try {
            return promoteService.add(promote);
    ...
    public ApiResponseResult getlist(Long id, Long bsItemsRecordId){
        try{
            ...
            return promoteService.getlist(id, bsItemsRecordId, super.getPageRequest(sort));
        }catch(BusinessException e){
            e.printStackTrace();
            return ApiResponseResult.failure(e.getMessage());
        }
