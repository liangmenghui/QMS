        ...
        <Card>
            ...
            <div>
                <Form 
                    ...
                    :rules="inforValidate"
                >
                    <FormItem label="用户账号：" prop="bsCode">
                        <span>{{userForm.bsCode}}</span>
                    </FormItem>
                    <FormItem label="用户姓名：" prop="bsName">
                        <div style="display:inline-block;width:300px;">
                            <Input v-model="userForm.bsName" ></Input>
                        </div>
                    </FormItem>
                    <FormItem label="邮箱：" prop="bsEmail">
                        <div style="display:inline-block;width:300px;">
                            <Input v-model="userForm.bsEmail" ></Input>
                        </div>
                    </FormItem>
                    <FormItem label="用户手机：" prop="bsMobile" >
                        <div style="display:inline-block;width:300px;">
                            <Input v-model="userForm.bsMobile" @on-keydown="hasChangePhone"></Input>
                        </div>
                        <!--
                    ...
                    <FormItem label="账号状态：">
                        <span v-if="userForm.bsStatus==1">已启用</span>
                        <span v-if="userForm.bsStatus==2">已停用</span>
                        <span v-if="userForm.bsStatus==3">已锁定</span>
                    </FormItem>
                    <!--
                    <FormItem label="公司：">
...
export default {
    ...
    methods: {
        getCurrentUser() {
            this.api.admin.user.current().then(res => {
                if (res.result) {
        ...
        saveEditPass () {
            this.$refs['editPasswordForm'].validate((valid) => {
                if (valid) {
                    ...
                    this.api.admin.user.editpassword(this.editPasswordForm).then(res=> {
                        if(res.result) {
        ...
        saveInfoAjax () {
            ...
            this.api.admin.user.edit(this.userForm).then(res=> {
                if(res.result) {
                    this.$Message.info(res.msg);
                }else {
                    this.$Message.error(res.msg);
