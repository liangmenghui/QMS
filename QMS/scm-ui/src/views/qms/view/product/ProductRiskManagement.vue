<template>
	...
	<div v-if="!showRooterView">
    	...
    	描述：质量经理批准
    --> 
    	<Row :style="{marginBottom: '20px'}" v-if="isApprover">
    		<Card>
				<p slot="title">
                    <Icon type="social-buffer" style="color: #ff9900;"></Icon>
                    &nbsp;{{$t('product.ChangeOfRiskLevel')}}
               	</p>
     			
				<el-form style="margin-left:5%">
					<el-input v-model="riskApprovedRecord.bsOriginalRiskLevel" size="small" readonly style="width:8%"></el-input>
					<Icon type="shuffle" style="font-size: 20px;padding:10px"></Icon>
					<el-input v-model="riskApprovedRecord.bsNewRiskLevel" size="small" readonly style="width:8%"></el-input>
					
					<p style="margin-top:10px;"></p>
					<el-radio v-model="riskApprovedRecord.bsStatus" label="1" border size="small">{{$t('risk.approval')}}</el-radio>
					<el-radio v-model="riskApprovedRecord.bsStatus" label="2" border size="small">{{$t('risk.reject')}}</el-radio>
              
                    <p class="mt10">{{$t('risk.approvalDesc')}}:</p>
                    <el-input type="textarea"  :autosize="{ minRows: 3, maxRows: 3}" style="width:50%" v-model="riskApprovedRecord.bsApprovedAdvice"></el-input>
					<p style="margin-top:10px;"></p>
					<el-button type="primary" style="width: 12%;" @click="approveRisk()">{{$t('risk.SubmitApprovalResult')}}</el-button>
				</el-form>
...
<!-- 		    		<el-table-column prop="" :label="$t('product.BusinessJudgement')">
			...
			<div style="margin: 10px;overflow: hidden">
				<div style="float: right;">
					<el-pagination @current-change="changePage" :current-page.sync="page"
						:page-size="rows" layout="prev, pager, next, jumper" :total="total">
					</el-pagination>
				</div>
			</div>
      		</Card>
      	...
      	<!-- <Row :style="{marginBottom: '20px'}">
        	<Card>
      		...
      		<div style="margin:25px 10px;">
      		    <div>
      			      <el-button type="primary" style="padding: 7px 5px;" @click="showChangeRiskGrade()">
    ...
    methods:{
    	getData(){
            if(this.$route.query.id != undefined) {
				...
				this.api.productinfo.getlist({id:this.productData.id}).then((res)=>{
                    this.productData = res.data.rows[0];
                    this.$store.commit("updateProductDataStates",res.data.rows[0]);
                    this.getRiskData();
                });
        ...
        getRiskData(){
            ...
            this.api.productrisk.getlist(params).then((res) => {
				...
				this.grrValues = [];
				this.riskScores = [];
                for(var i = this.productrisk.length - 1; i >= 0; i--) {
					this.productrisk[i].bsFlowRecordId = this.productData.bsProcessRecordId;
				...
				if(this.productrisk[0].bsStatus == 1){
					this.api.riskApprovedRecord.getlist({bsPrId:this.productData.id,bsStatus:0}).then((res) => {
						this.riskApprovedRecord = res.data.rows[0];
						if(this.riskApprovedRecord.approvedBy.bsCode == Cookies.get('user')){
		...
		approveRisk(){
			if(this.riskApprovedRecord.bsStatus == 0){
				this.$Message.error('请选择结果');
			}else{
				this.api.riskApprovedRecord.approve(this.riskApprovedRecord).then((res) => {
					if(res.result){
						this.$Message.info('成功提交批准结果');
						this.getData();
					}
				});
			}
    	...
    	pushToInputInformation(url){
    		...
    		this.$store.commit("updateRiskDataStates",this.productrisk[0]);       	
      	},
      	toInstructions(url){
            this.showRooterView = true;
      		this.$router.push({path:url,query:this.$route.query});
        ...
        submitapplication(){    //提交变更申请
         var parm={
            ...
            bsRiskLevelUser:this.productData.bsRiskLevelUser,
            bsRiskDescUser:this.productData.bsRiskDescUser
         }
          ...
          this.api.productinfo.completeRiskLevel(parm).then((res) => {            
            if(res.result) {
                  this.productData.bsRiskLevel=this.productData.bsRiskLevelUser; 
        ...
        switchingMode(){
            ...
            this.api.productinfo.autoRiskLevel(parm).then((res) => {
               if(res.result) {               
                  ...
                  this.productData.bsRiskLevel=this.productData.bsRiskManual?this.productData.bsRiskLevelUser:this.productData.bsRiskLevelSys;
              }          
