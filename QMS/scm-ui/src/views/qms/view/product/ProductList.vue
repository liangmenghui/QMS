...
<!--搜索框 -->
          ...
          <!--   <Select size="large" style="width: 200px;display: inline-block;margin-top: -4px;" v-model="formQuery.bsStatus" placeholder="按产品状态搜索">
			...
			<Select size="large" style="width: 90px;display: inline-block;margin-top: -4px;" v-model="formQuery.bsApprovedType" v-if="isApprovedType">
    			<Option key="11" :label="$t('product.ProductAudit')" value="11"></Option>
    			<Option key="22" :label="$t('product.ProductInspection')" value="12"></Option>
  			</Select>
            ...
            <el-table-column type="expand">
                <template slot-scope="props">
                    <el-form label-position="left" inline class="demo-table-expand">
						...
						<!--
						<el-button v-perm-verify="" type="primary" size="small" :disabled="props.row.disabledProcessRecord" @click="pushToProductArrovedFlow('product/approved',props)">{{$t('product.ProductAudit')}}
                        </el-button>
                        <el-button v-perm-verify="" type="primary" size="small" :disabled="props.row.disabledProductRecord" @click="pushToProdcutionCheckFlow('product/approved',props)">{{$t('product.ProductInspection')}}
                        </el-button>
                        <el-button v-perm-alloc="" type="primary" size="small" :disabled="props.row.bsIsApprove==1" @click="pushToApprovedCreator('product/createApprovedRecord',props)">{{$t('product.NewAudit')}}
                        </el-button>
						-->
						
                        <el-button v-perm-query="" type="primary" size="small" @click="pushToChildWithData('product/approvedRecordsList',props)">{{$t('product.ProductAudit')}}</el-button>
                    </el-form>    
                </template>
            </el-table-column>
  <!--   数据表格 -->
            <el-table-column align="left"
                :label="$t('product.RiskLevel')" width="80px"
                prop="bsRiskLevel">
            </el-table-column>
            <!-- <el-table-column align="left"
			...
			<el-table-column
              :label="$t('product.organization')" 
              prop="bsOrganizationName">
            </el-table-column> 
            <el-table-column
                :label="$t('product.SuppChieseName')" 
...
export default {
    ...
    created(){
		var isApproved = Object.assign({},this.$route.query);
		if(isApproved.bsIsApprove == undefined){
			this.formQuery.bsApprovedType = '';
    	...
    	getData(){
    		...
    		this.api.productinfo.getlist(filter).then((res) => {
          		...
          		this.tableData = res.data.rows.map(function(row) {
                    ...
                    row.disabledProductRecord = true;
                    row.disabledProcessRecord = true;
                    if(row.bsProcessRecordId == null){
                       row.bsProcessRecordId = 0;
                    }
					if(row.bsProductRecordId == null){
                       row.bsProductRecordId = 0;
                    }					

                    if(row.bsIsApprove == 1 && row.bsProcessRecordId>row.bsProductRecordId){
						row.disabledProcessRecord = false;
					}else if(row.bsIsApprove == 1 && row.bsProcessRecordId<row.bsProductRecordId){
						row.disabledProductRecord = false;
					}
                    return row;
                });
