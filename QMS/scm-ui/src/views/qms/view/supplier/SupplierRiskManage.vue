	...
	<div v-if="!showRooterView">
		...
		<div style="height: 40px;">
			<div style="float:right"> 
				...
				<!--
      			<el-button type="success" @click="pushToInputInformation('riskManagement/inputInformation')" style="padding: 8px 20px;margin-left: 10px;" v-if="$Util.hasPerm('EDIT')">
    	...
    	<Row :style="{marginBottom: '20px'}" v-if="isApprover">
    		<Card>
				...
				<el-form style="margin-left:5%">
					<el-input v-model="riskApprovedRecord.bsOriginalRiskLevel" readonly style="width:15%"></el-input>
					<Icon type="shuffle" style="font-size: 20px;padding:10px"></Icon>
					<el-input v-model="riskApprovedRecord.bsNewRiskLevel" readonly style="width:15%"></el-input>
					
					<p style="margin-top:10px;"></p>
					<el-radio v-model="riskApprovedRecord.bsStatus" label="1" border>{{$t('risk.approval')}}</el-radio>
					<el-radio v-model="riskApprovedRecord.bsStatus" label="2" border>{{$t('risk.reject')}}</el-radio>
              
                    <p class="mt10">{{$t('risk.approvalDesc')}}:</p>
                    <el-input type="textarea"  :autosize="{ minRows: 6, maxRows: 10}" style="width:50%" v-model="riskApprovedRecord.bsApprovedAdvice"></el-input>
					<p style="margin-top:10px;"></p>
    ...
    methods:{
        getData(){
            if(this.$route.query.id != undefined) {
                ...
                this.api.supplierinfo.getlist({id:this.supplierData.id}).then((res)=>{
                    this.supplierData = res.data.rows[0];
                    this.$store.commit("updateSupplierDataStates",res.data.rows[0]);
                    this.getRiskData();
                });
    	...
    	getRiskData(){
    		...
    		this.api.supplierrisk.getlist(params).then((res) => {
				...
				this.riskScores = [];
      			for(var i = this.supplierrisk.length - 1; i >= 0; i--) {
					this.supplierrisk[i].bsFlowRecordId = this.supplierData.bsSuppRecordId;
				...
				/*if(this.supplierrisk[0].bsStatus == 1){
					this.api.riskApprovedRecord.getlist({bsSuppId:this.supplierData.id,bsStatus:0}).then((res) => {
