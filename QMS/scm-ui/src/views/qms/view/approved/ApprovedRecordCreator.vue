		...
		<div class="ivu-col-span-10" style="margin:4%">			
            <el-form ref="form" :model="form" label-width="100px">
                ...
                <el-form-item :label="$t('ApprovedFlow.Principal')">
					<el-select style="width:379px;" multiple v-model="selectedUsers" value-key="uid" placeholder="请选择">
						<el-option v-for="user in users" :key="user.uid" :label="user.user_name" :value="user" ></el-option>
					</el-select>
				</el-form-item>
				
				<el-form-item :label="$t('ApprovedFlow.PlanDate')">
					<div>                          
						<el-date-picker style="width:380px"
						  v-model="selectedFlow.bsPlanDate"
						  type="date"
				...
				<el-form-item :label="$t('ApprovedFlow.productType')" v-if="form.bsPrId != undefined">
					<el-select style="width:379px;" v-model="selectedFlow.bsProductType" placeholder="请选择">
...
export default {
    data() {
        return {
            ...
            users :[],
			Options:[
				{label:this.$t("ApprovedFlow.productTypeArr[0]"),value:0},
				{label:this.$t("ApprovedFlow.productTypeArr[1]"),value:1},
				{label:this.$t("ApprovedFlow.productTypeArr[2]"),value:2},
				{label:this.$t("ApprovedFlow.productTypeArr[3]"),value:3},
				{label:this.$t("ApprovedFlow.productTypeArr[4]"),value:4}
            ],
    ...
    methods: {         
        getData(type){  
			...
			if(form.bsPrId != undefined){
				//查询SQE人员
				this.api.ApprovedFlow.getUserByBsType({bsType:1}).then((res)=>{
					this.users = res.data;
				});
			}else if(form.bsSuppId != undefined){
				//查询SDE人员
				this.api.ApprovedFlow.getUserByBsType({bsType:2}).then((res)=>{
					this.users = res.data;
				});
			}
        },
        ...
        createApporvedRecord(){
			...
			this.form.bsPlanDate = this.selectedFlow.bsPlanDate;
            this.form.recorderStr=this.selectedUsers.map(function (user) {
				return user.uid;
			}).toString();
			this.form.bsProductType = this.selectedFlow.bsProductType;
