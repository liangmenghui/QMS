    ...
    <div class="approvedextrareports">
       <Card class="InitialReview">
        <div slot="title" class="title3">
            ...
            <el-button class="btn" type="primary" plain  size="small" icon="el-icon-warning" @click='exportExcel()' v-if="perms.export">{{$t('approved.export')}}</el-button>
        </div >
        ...
        <Row style="margin:15px 0">
			...
			<Card>
                ...
                <div id="other-body">  
                    <!-- 初期评审才显示供应商等级 -->
					<div class="approvedConclusion" v-if="approvedItemRecord.bsSuppId&&(flowData.bsType==22||flowData.bsType==21)">
					...
					<div class="approvedConclusion" v-if="approvedItemRecord.bsPrId&&flowData.bsType==11">
                        <Col :md="8"><p class="grade">{{$t('New-audit.approvedResult')}}: <span class="supplevel">{{approvedResult}}</span></p></Col>   
                        <Col :md="8"><p class="grade">{{$t('New-audit.score_percent')}}: <span class="supplevel">{{percentage.toFixed(2)+'%'}}</span></p></Col>         
                    </div>
             ...
             <Card class="cards">
                ...
                <div id="other-body">  
                    <el-table :data="promoteData" style="width: 100%;">
						...
						<el-table-column prop="bsResponsible" :label="$t('ApprovedFlow.Principal')">
						</el-table-column>  
						...
						<el-table-column :label="$t('Button.operating')">
							<template slot-scope="scope">
								...
								<el-button size="mini" type="warning" plain @click="delpromote(scope)" :disabled="!perms.edit">{{$t('Button.Delete')}}</el-button>
							</template>
						</el-table-column>
                    </el-table>                  
                </div>                
   ...
   <!--  新增 -->
        <!--   <Modal class="promotModel" v-model="promoteModel.modal_dialog" :title="$t('supplier.addImprovementItems')"   okText='新增' cancelText="取消"  @on-ok="addSubmit" @on-cancel="cancel" >                
           <el-form ref="addForm" :model="addForm" :rules="addFormRules" label-width="120px">
				...
				<el-form-item :label="$t('ApprovedFlow.Principal')">
					<el-input v-model="addForm.bsResponsible"></el-input>
				</el-form-item>
		...
		<Modal class="promotModel" v-model="editFormVisible" :title="$t('supplier.addImprovementItems')"   okText='提交' cancelText="取消"  @on-ok="editSubmit" @on-cancel="cancel" >                
           <el-form ref="editForm" :model="editForm" :rules="editFormRules" label-width="120px" >
			  ...
			  <el-form-item :label="$t('ApprovedFlow.Principal')">
					<el-input v-model="editForm.bsResponsible"></el-input>
				</el-form-item>
              <el-form-item :label="$t('supplier.FinishTime')">               
                <el-date-picker
...
export default {
    ...
    created(){
		...
		this.perms.export = this.approvedItemRecord.users.length == this.approvedItemRecord.bsStep || this.approvedItemRecord.bsStatus != 1;
    },
    methods: {    
        formatPercent(row){
     ...
     //显示新增
        handleAdd(){
            ...
            this.addForm={
				...
				bsResponsible:this.addForm.bsResponsible,
    ...
    //删除列表
		...
		//导出
        exportExcel(){
			if(this.approvedItemRecord.bsPrId != undefined){
				this.api.fileQms.getProductExcel({bsFlowRecordId:this.$route.query.bsFlowRecordId}).then((link)=>{
					//link.click(); 
				});
			}else{
...
.promotModel .ivu-modal-body{width: 90%;font-family: "Microsoft YaHei"}
 ...
 .td .el-upload--text{
    ...
    .approvedConclusion{display: inline-block; width:100%; }
    .approvedConclusion .grade{line-height: 35px;font-family: "Microsoft YaHei";font-size:16px;}
