        	...
        	<Card>
				<p slot="title">
					...
					<div style="position:absolute;top:10px;left:160px;">
						<el-select v-model="bsPrName" size="small" :placeholder="$t('product.TheProportionOfHigh-riskProducts')">
							<el-option v-for="item in productDanger" :key="item.id" :label="item.bsPrName" :value="item.bsPrName">
							</el-option>
						</el-select>
					</div>
                </p>
...
<script>
    ...
    methods:{
		getData(){
			...
			//获取高风险产品
			var bsRiskLevel = 5;
			this.api.productinfo.getlist({suppCode:this.supplierinfo.bsSuppCode,bsRiskLevel:bsRiskLevel}).then((res) => {
				this.productDanger = res.data.rows;
            });
