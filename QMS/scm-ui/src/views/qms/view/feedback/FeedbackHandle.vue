...
-->
        <Collapse accordion v-model="expandedCollapses[0]">
            <Panel name="1">
                ...
                <div class="title2">{{$t('feedback.CustomerInformation')}}</div>
                <div id="product-body" slot="content">
					<div class="product-body-div">{{$t('feedback.NumberOfQuestions')}}：<span>{{form.id}}</span></div>
					<div class="product-body-div">{{$t('feedback.CustomerID')}}：<span>{{form.bsCusCompanyCode}}</span></div>
                    <div class="product-body-div">{{$t('feedback.CusCompanyName')}}：<span>{{form.bsCusCompanyName}}</span></div>
...
-->
                ...
                <Panel name="1">
                    ...
                    <div id="measures-body" slot="content">
                        ...
                        <el-table :data="handlers[index]" style="width: 100%;" v-if="index==4">
                            ...
                            <el-table-column prop="handlerBy.bsActionType" :label="$t('feedback.type')" :formatter="formatActionType">
                            </el-table-column>
        ...
        <div id="bottom">
            <center>
                ...
                <el-button type="warning" v-if="perms.export" @click="exportExcel" style="padding: 7px 20px;">
        ...
        <Modal v-model="modal_dialog" :title="modal_form.title" @on-ok="saveHandler">
            <div class="modeform">
                <el-form label-width="120px">
                    ...
                    <el-form-item :label="$t('feedback.type')" v-if="modal_form.bsType==4">
                        <el-select style="width:300px;" v-model="modal_form.bsActionType" :placeholder="$t('choose')">
                            <el-option :label="$t('feedback.actionTypes[1]')" :value="1">
                            </el-option>
                            <el-option :label="$t('feedback.actionTypes[2]')" :value="2">
                            </el-option>
                            <el-option :label="$t('feedback.actionTypes[3]')" :value="3">
                            </el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item :label="$t('approved.principal')" v-if="modal_form.bsType!=4">
                        <el-select style="width:300px;" v-model="selectedUser" value-key="id" filterable :placeholder="$t('choose')">
...
export default {
    ...
    methods: {
        getData() {
            ...
            this.api.Audit.getuser().then((res) => {             
        ...
        gethandlers(callback) {
            this.api.feedback.gethandlers({ bsFeedbackId: this.form.id }).then((res) => {
                ...
                for (var i = 0; i < res.data.rows.length; i++) {
                    ...
                    if(row.bsType == 2) this.analysis = row;
                ...
                for (var i = this.handlers[1].length - 1; i >= 0; i--) {
                    ...
                    if(handler.handlerBy != undefined) this.userSelections.push(handler.handlerBy);
                        ...
                        bsEmail:handler.bsHandlerEmail
                    };
                }
        ...
        markActionFinished() {
            ...
            this.saveHandler(function() {
                _this.markTodoFinished(uid, function(data) {
                    //_this.excuteActions(_this.form.bsType);
					_this.excuteActions(_this.modal_form.bsStep);
        ...
        submitAnalysis() {
			...
			this.selectedUser = {"id":Cookies.get("uid")};
            this.modal_form = {
        ...
        saveHandler(callback) {
            ...
            if (this.modal_form.bsType == 1) {
                for (var i = this.handlers[1].length - 1; i >= 0; i--) {
                    ...
                    if (row.handlerBy.id != undefined && row.handlerBy.id == this.modal_form.bsHandlerId){
        ...
        formatActionType(row, column) {
            return row.bsActionType==undefined?"":this.$t('feedback.actionTypes['+row.bsActionType+']');
        },
        exportExcel(file) {
            this.api.fileQms.getFeedbackExcel({bsFeedbackId:this.form.id} ).then((res) => {               
                //res.click(); 
            })
        }
