<template>
    ...
    <div class="feedbacklist" v-if="!showRooterView">   	
    	<div>
             <Select size="large" filterable style="width: 200px;display: inline-block;margin-top: -4px;" v-model="formQuery.bsStatus" placeholder="按客诉状态搜索">
			...
			<el-table-column type="expand">
			  <template slot-scope="props">
				<el-form label-position="left" inline class="demo-table-expand">
					<el-button type="primary" size="small" @click="to('feedback/report',props.row.id)" v-if="props.row.bsStatus==0 && props.row.bsType==1">{{$t('feedback.editFeedback')}}</el-button>
					<el-button type="primary" size="small" @click="to('feedback/details',props.row.id)" v-if="props.row.bsType==1">{{$t('feedback.details')}}</el-button>
					<el-button type="primary" size="small" @click="to('feedback/handle',props.row.id)" v-if="props.row.bsType==1">{{$t('feedback.dispose')}}</el-button>
					<el-button type="primary" size="small" v-if="props.row.bsType==2">{{$t('feedback.details')}}</el-button>
				</el-form>
			  </template>
			</el-table-column>
			<el-table-column
			  :label="$t('feedback.number')" width="80" 
			  prop="id">
			</el-table-column>
			<el-table-column
			  :label="$t('feedback.state')" width="100" >
				<template scope="scope">
					<span v-bind:class="{grey:scope.row.bsStatus==0, red:scope.row.bsStatus==1, blue:scope.row.bsStatus==2, green:scope.row.bsStatus==3}">{{$t('feedback.statusDesc['+(scope.row.bsStatus)+']')}}</span>
				</template>
			</el-table-column>
			<el-table-column
			  :label="$t('feedback.type')" width="100">
				<template scope="scope">
					<span>{{$t('feedback.feedbacktype['+(scope.row.bsType)+']')}}</span>
				</template>
			</el-table-column>
			<el-table-column :label="$t('feedback.CusCompanyName')" prop="bsCusCompanyName">
			</el-table-column>
			<el-table-column :label="$t('feedback.CustomerID')" prop="bsCusCompanyCode">
			</el-table-column>
			<el-table-column
...
<script>
    ...
    data() {
        return {
            ...
            options: [{
                value: '1',
                label: this.$t('feedback.statusDesc[1]')
            }, {
                value: '2',
                label: this.$t('feedback.statusDesc[2]')
            }, {
                value: '3',
                label: this.$t('feedback.statusDesc[3]')
            }],  
        }
    },
    created(){
