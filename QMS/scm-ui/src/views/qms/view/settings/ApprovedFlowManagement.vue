    ...
    <div class="ApprovedFlowManagement" v-if="!showRooterView">
            <div>
				...
				<Modal v-model="dialog.modal_dialog" :title="$t('ApprovedFlow.ProcessInformation')" @on-ok="ok" @on-cancel="cancel" >
					<el-form ref="dialog.formItem" :model="dialog.formItem" label-position="left" :rules="dialog.ruleForm" label-width="80px" class="addform">
					   ...
					   <el-form-item :label="$t('ApprovedFlow.Principal')" prop="bsApprovederId">
							<el-select v-model="selecedtUser" placeholder="请输入" value-key="id">
								<el-option v-for="user in users" :label="user.bsName" :value="user" :key="user.id"></el-option>
							</el-select>
						</el-form-item>
				   ...
				   <!--      <el-form-item :label="$t('approved.can_yield')" prop="bsCanYield" >
						...
						<el-form-item :label="$t('approved.ThroughTheState')" prop="bsApprovedStatus" >
						...
						<el-form-item :label="$t('approved.NotAState')" prop="bsRejectStatus" >                            
...
export default {
    ...
    methods: {
        ...
        showEditDialog(params) {   
            ...
            this.selecedtUser = Object.assign({},this.dialog.formItem.approvedBy);
                ...
                if (valid) {
                    if(typeof(this.dialog.formItem.id)!=undefined && typeof(this.dialog.formItem.id)=="number") {
                        ...
                        this.api.ApprovedFlow.add(this.dialog.formItem).then((res) => {
                            res.data.approvedBy = this.selecedtUser;
