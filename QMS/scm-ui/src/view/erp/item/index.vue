            ...
            <i-col>
                <Form class="query_area" ref="formQuery" :model="formQuery" inline>
                    <Form-item prop="bsCode">
                        <Input type="text" v-model="formQuery.bsCode" placeholder="物料编码" />
                    </Form-item>
                    <Form-item prop="bsName">
                        <Input type="text" v-model="formQuery.bsName" placeholder="物料名称" />
                    </Form-item>
                    <Form-item v-if="true">
        ...
        <Modal v-model="dialog.modal_dialog" title="" @on-ok="ok" @on-cancel="cancel" width="800">
            <p>
                ...
                <Row>
                    ...
                    <i-col span="12">
                        <Form-item label="物料编码" prop="bsCode">
                ...
                <Row>
                    <i-col span="24">
                        <Form-item label="物料名称" prop="bsName">
                ...
                <Row>
                    <i-col span="12">
                        <Form-item label="类别ID" prop="bsCategory">
                            <Input v-model="dialog.formItem.bsCategory" placeholder="请输入"></Input>
                        </Form-item>
                    </i-col>
                    <i-col span="12">
                        <Form-item label="类别名称" prop="bsCategoryDesc">
                            <Input v-model="dialog.formItem.bsCategoryDesc" placeholder="请输入"></Input>
                        </Form-item>
                    </i-col>
                <Row>
                </Row>
                    <i-col span="12">
                        <Form-item label="类型ID" prop="bsItemType">
                            <Input v-model="dialog.formItem.bsItemType" placeholder="请输入"></Input>
                        </Form-item>
                    </i-col>
                    <i-col span="12">
                        <Form-item label="类型名称" prop="bsItemTypeDesc">
                            <Input v-model="dialog.formItem.bsItemTypeDesc" placeholder="请输入"></Input>
                        </Form-item>
                    </i-col>
                </Row>
                </Form>
...
export default {
    ...
    methods: {
        ...
        getData() {
            ...
            this.api.erp.item.getlist(this.formQuery).then((res) => {
                if(res.result) {
        ...
        edit(params) {
            this.api.erp.item.edit(params.row).then((res)=>{
                if(res.result) {
                    //refresh
                    this.reloadData();
                }else {
        ...
        delete(params) {
            this.$Modal.confirm({
                ...
                loading: true,
                onOk: () => {
                    this.api.erp.item.delete({id:params.row.id}).then((res)=>{
                        if(res.result) {
                            //refresh
                            this.reloadData();
        ...
        download() {
            this.api.erp.item.download(this.dialog.formItem).then((res) => {
                if(res.result) {
                    //refresh
                    this.reloadData();
                }else {
        ...
        ok () {
            if(typeof(this.dialog.formItem.id)!=undefined && typeof(this.dialog.formItem.id)=="number") {
                ...
                //     if (valid) {
                        this.api.erp.item.edit(this.dialog.formItem).then((res) => {
                            if(res.result) {
                                //refresh
                                this.reloadData();
                            }else {
                ...
                // this.$refs["dialog.ruleForm"].validate((valid) => {
                //     if (valid) {
                        this.api.erp.item.add(this.dialog.formItem).then((res) => {
                            if(res.result) {
                                //refresh
                                this.reloadData();
                            }else {
