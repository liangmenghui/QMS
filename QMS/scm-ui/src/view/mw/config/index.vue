    ...
    <div>
        ...
        <Modal v-model="dialog.modal_dialog" title="" @on-ok="ok" @on-cancel="cancel" width="800" >
            <p>
                ...
                <Row>
                    <i-col span="12">
                    <Form-item label="转换类（In）" prop="bsParameterInCls">
                        <Input v-model="formItem.bsParameterInCls" placeholder="请输入"></Input>
                    </Form-item>
                    </i-col>
                    <i-col span="12">
                    <Form-item label="业务处理类" prop="bsBusiCls">
                        <Input v-model="formItem.bsBusiCls" placeholder="请输入"></Input>
                    </Form-item>
                    </i-col>
                </Row>
                <Row>
                    <i-col span="24">
                    <Form-item label="回调URI" prop="bsRedirectUrl">
                        <Input v-model="formItem.bsRedirectUrl" placeholder="请输入"></Input>
                    </Form-item>
                    </i-col>
                </Row>
                <Row>
...
export default {
    ...
    methods: {
        ...
        getData() {
            ...
            this.api.mw.apiconfig.getlist(this.formQuery).then((res) => {
                if(res.result) {
        ...
        edit(params) {
            this.api.mw.apiconfig.edit(params.row).then((res)=>{
                if(res.result) {
                    //refresh
                    this.reloadData();
                }else {
        ...
        delete(params) {
            this.$Modal.confirm({
                ...
                loading: true,
                onOk: () => {
                    this.api.mw.apiconfig.delete({id:params.row.id}).then((res)=>{
                        if(res.result) {
                            //refresh
                            this.reloadData();
        ...
        showEditDialog(params) {
            this.dialog.modal_dialog = true;
            let r = params.row;
            this.formItem = {id:r.id,bsCode:r.bsCode,bsName:r.bsName,bsRedirectUrl:r.bsRedirectUrl,bsParameterInCls:r.bsParameterInCls,bsBusiCls:r.bsBusiCls,bsRevision:r.bsRevision,bsEnableState:r.bsEnableState,bsPublishDes:r.bsPublishDes,bsComment:r.bsComment};
        ...
        ok () {
            ...
            if(typeof(this.formItem.id)!=undefined && typeof(this.formItem.id)=="number") {
                this.$refs["dialog.ruleForm"].validate((valid) => {
                     if (valid) {
                        this.api.mw.apiconfig.edit(this.formItem).then((res) => {
                            if(res.result) {
                                //refresh
                                this.reloadData();
                            }else {
                ...
                this.$refs["dialog.ruleForm"].validate((valid) => {
                    if (valid) {
                        this.api.mw.apiconfig.add(this.formItem).then((res) => {
                            if(res.result) {
                                //refresh
                                this.reloadData();
                            }else {
        ...
        testInvoke(params) {
            ...
            this.api.mw.apiconfig.test(data, params.row.bsRevision, params.row.bsCode).then((res) => {
                if(res.result) {
