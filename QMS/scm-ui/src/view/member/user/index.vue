    ...
    <div>
        ...
        <Modal v-model="dialog.modal_dialog" title="" @on-ok="ok" @on-cancel="cancel" width="800">
            <p>
                ...
                <Row>
                        <i-col span="12">
                            <Form-item label="关联人员信息" prop="pkPerson">
                                 <Select v-model="dialog.formItem.pkPerson"  >
                                            <Option v-for="item in personList"  :key="item.id" :value="item.id"  >{{item.bsName}}</Option>
                                 </Select>
                            </Form-item>
                        </i-col>
                    </Row>
...
export default {
    ...
    methods: {
        ...
        resetpassword(params) {
            this.$Modal.confirm({
                title: '提示信息',
                content: '<p>是否确定重置登录账号为：'+params.row.bsCode+'的登录密码？</p>',
                onOk: () => {
