<template>
    <div>
                ...
                <Form :model="dialog.formItem" :ref="dialog.ruleForm" :rules="dialog.ruleForm" :label-width="80">
                    ...
                    <Form-item label="所属角色组" prop="pkSysRolegroup">
                        <Select v-model="dialog.formItem.pkSysRolegroup" placeholder="请输入">
                            <Option v-for="item in dialog.refs.rolegroup" :value="item.id" :key="item.bsCode">{{item.bsCode}} - {{item.bsName}}</Option>
                        </Select>
                    </Form-item>
        ...
        <!--分配权限-->
        <Modal v-model="perms.modal_dialog" title="分配权限" @on-ok="save" @on-cancel="unsave" width="800" height="600" >
            <p>
                <input type="hidden" id="roleId" name="roleId" value="" />
                <Tree ref="resrce_tree" :data="perms.tree.data" :render="renderTree" class="layout-menu-left"  show-checkbox></Tree>
            </p>
        </Modal>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        getData() {
            ...
            this.api.admin.role.getlist(this.formQuery).then((res) => {
                if(res.result) {
        ...
        edit(params) {
            this.api.admin.role.edit(params.row).then((res)=>{
                if(res.result) {
                    //refresh
                    this.reloadData();
                }else {
        ...
        delete(params) {
            this.$Modal.confirm({
                ...
                loading: true,
                onOk: () => {
                    this.api.admin.role.delete({id:params.row.id}).then((res)=>{
                        if(res.result) {
                            //refresh
                            this.reloadData();
        ...
        ok () {
            if(typeof(this.dialog.formItem.id)!=undefined && typeof(this.dialog.formItem.id)=="number") {
                ...
                //     if (valid) {
                        this.api.admin.role.edit(this.dialog.formItem).then((res) => {
                            if(res.result) {
                                //refresh
                                this.reloadData();
                            }else {
                ...
                // this.$refs["dialog.ruleForm"].validate((valid) => {
                //     if (valid) {
                        this.api.admin.role.add(this.dialog.formItem).then((res) => {
                            if(res.result) {
                                //refresh
                                this.reloadData();
                            }else {
        ...
        getRolegroup() {
            this.api.admin.rolegroup.getlist({}).then((res) => {
                if(res.result) {
        ...
        getlist(pkRole) {
            ...
            this.api.admin.roleperm.getlist({pkRole:pkRole}).then((res) => {
                if(res.result) {
                    this.spellTitle(res.data.resrces, res.data.perms, res.data.alloced);
                    this.perms.tree.data = res.data.resrces;
                    Object.assign(this.resrceMap, res.data.resrceMap);
        ...
        getPerms() {
            ...
            for(let i = 0; i < nodes.length; i++) {
                if(nodes[i].parent) {
                    this.addParent(res, nodes[i].parent.id);
                }
                let perm = {};
        ...
        addParent(res, id) {
            let parent = this.resrceMap[id];
            if(!parent) {
                return;
            }
            this.addParent(res, parent.pkParent);
            res.push({pkResrce: parent.id});
        },
        save() {
            let res = this.getPerms();
            let roleId = document.getElementById("roleId").value;
            this.api.admin.roleperm.save({roleId:roleId, rrpvo: JSON.stringify(res)}).then((res) => {
                if(res.result) {
