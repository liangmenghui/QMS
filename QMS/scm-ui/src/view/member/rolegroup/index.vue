...
export default {
    ...
    methods: {
        ...
        getData() {
            ...
            this.api.admin.rolegroup.getlist(this.formQuery).then((res) => {
                if(res.result) {
        ...
        edit(params) {
            this.api.admin.rolegroup.edit(params.row).then((res)=>{
                if(res.result) {
                    //refresh
                    this.reloadData();
                }else {
        ...
        delete(params) {
            this.$Modal.confirm({
                ...
                content: '<p>是否确定删除？</p>',
                onOk: () => {
                    this.api.admin.rolegroup.delete({id:params.row.id}).then((res)=>{
                        if(res.result) {
                            //refresh
                            this.reloadData();
                            this.$Modal.remove();
        ...
        ok () {
            if(typeof(this.dialog.formItem.id)!=undefined && typeof(this.dialog.formItem.id)=="number") {
                ...
                //     if (valid) {
                        this.api.admin.rolegroup.edit(this.dialog.formItem).then((res) => {
                            if(res.result) {
                                //refresh
                                this.reloadData();
                            }else {
                ...
                // this.$refs["dialog.formItem"].validate((valid) => {
                //     if (valid) {
                        this.api.admin.rolegroup.add(this.dialog.formItem).then((res) => {
                            if(res.result) {
                                //refresh
                                this.reloadData();
                            }else {
