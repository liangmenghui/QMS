...
export default {
    ...
    data() {
        return {
            ...
            datagrid: {
                ...
                columns: [
                    {
                        title: this.$t('systemInfo.resourceManage.resourceCoding'),
                        key: 'bsCode'
                    },
                    {
                        title: this.$t('systemInfo.resourceManage.resourceName'),
                        key: 'bsName'
                    },
                    {
                        title: this.$t('systemInfo.resourceManage.resourceType'),
                        key: 'bsLeaf',
                        render: (h, params) => {
                    ...
                    {
                        title: this.$t('systemInfo.resourceManage.url'),
                        key: 'bsUrl'
                    },
                    {
                        title: this.$t('systemInfo.resourceManage.state'),
                        key: 'bsEnableState',
                        align: 'center',
                        render: (h, params) => {
                    ...
                    {
                        title: this.$t('systemInfo.resourceManage.deleteFlag'),
                        key: 'bsIsDel',
                        render: (h, params) => {
                            return h('span', {
                                    style: {
                    ...
                    {
                        title: this.$t('systemInfo.resourceManage.createTime'),
                        key: 'bsCreatedTime'
                    },
                    {
                        title: this.$t('systemInfo.resourceManage.updateTime'),
                        key: 'bsModifiedTime'
                    },
                    {
                        title: this.$t('systemInfo.resourceManage.action'),
                        key: 'action',
                        render: (h, params) => {
                            let buttons = [];
                            if(this.menuData.perms.ALLOC) {
                                if(params.row.bsLeaf == 1) {
                                    ...
                                    }, this.$t('button.permissionSetting')));
                                }
                            }
    ...
    methods: {
        ...
        getData() {
            ...
            this.api.admin.resrce.getlist(this.formQuery).then((res) => {
                if(res.result) {
                ...
                onOk: () => {
                    this.api.admin.resrce.delete({id:params.row.id}).then((res)=>{
                        if(res.result) {
                            ...
                            this.getTree();
                            this.reloadData();
                            this.$Message.info(this.$t("message.sucess"));
                            this.$Modal.remove();
                        }else {
                            this.$Message.error(res.msg);
                        }
        ...
        getTree() {
            this.api.admin.resrce.gettree({}).then((res) => {
        ...
        getPerms(params) {
            let r = params.row;
            this.api.admin.resrceperm.getlist({pkResrce:r.id}).then((res) => {
                if(res.result) {
        ...
        perms_ok() {
            ...
            this.api.admin.resrceperm.save({pkResrce:this.perms.formItem.pkResrce, pkPerms: pkPerms}).then((res) => {
                if(res.result) {
                    this.$Message.info(this.$t("message.sucess"));
                }else {
                    this.$Message.error(this.$t("message.failure"));
                }
            });
        },
        perms_cancel() {
