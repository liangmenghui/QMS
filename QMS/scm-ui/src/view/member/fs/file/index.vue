        ...
        <Modal v-model="dialog.modal_dialog" title="" @on-ok="ok" @on-cancel="cancel" >
            <p>
                <Form :model="dialog.formItem" :rules="dialog.ruleForm" :label-width="80" enctype="multipart/form-data" >
                    ...
                    <Input v-model="dialog.formItem.pkFsCatalog" value="-1" ></Input>
                    </span>
...
export default {
            ...
            datagrid: {
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        key: 'bsFilePath',
                        render: (h, params) => {
                            return h('a', {on:{click: () => {window.open('/admin/fs/file/get/'+params.row.id+"/?filepath="+params.row.bsFilePath+"/"+params.row.bsFileName)}}}, params.row.bsFilePath + '/' + params.row.bsFileName)
                        }
                    },
                    {
                        title: '删除标识',
                    ...
                    },
                    {
                        ...
                        render: (h, params) => {
                            ...
                            if(this.menuData.perms.DOWNLOAD) {
                                buttons.push(h('Button', {
                                    props: {
                                        type: 'primary',
                                        size: 'small'
    ...
    methods: {
        getGroupList() {
            this.api.admin.ref.grouplist({}).then((res) => {
        ...
        getOrgList(pkGroup) {
            this.api.admin.ref.orglist({pkGroup:pkGroup}).then((res) => {
                //console.log("get list===>");
                //console.log(res.data);
                if(res.result) {
        ...
        getData() {
            ...
            this.api.fs.fsfile.getlist(this.formQuery).then((res) => {
                // //console.log("get list===>");
                // //console.log(res.data);
                if(res.result) {
        ...
        delete(params) {
            ...
            this.$Modal.confirm({
                ...
                loading: true,
                onOk: () => {
                    this.api.fs.fsfile.delete({id:params.row.id}).then((res)=>{
                        if(res.result) {
                            this.getTree();
                            this.reloadData();
        ...
        ok () {
            ...
            if(typeof(this.dialog.formItem.id)!=undefined && typeof(this.dialog.formItem.id)=="number") {
                ...
                //     if (valid) {
                        this.api.fs.fsfile.edit(formData).then((res) => {
                            if(res.result) {
                                this.reloadData();
                            }else {
                                this.$Message.error(res.msg);
                ...
                // this.$refs["dialog.ruleForm"].validate((valid) => {
                //     if (valid) {
                        this.api.fs.fsfile.add(formData).then((res) => {
                            if(res.result) {
                                //refresh
                                this.reloadData();
                            }else {
